...
function getStatusIcon(start, end) {
  if (start && !end) return "🕒";
  if (start && end) return "✅";
  return "";
}
...
function renderTable() {
  ...
  routines.forEach((r, index) => {
    ...
    const status = getStatusIcon(start, end);
    let noteInput = "";
    if (r.note) {
      noteInput = `<textarea 
        id="note-${index}" 
    ...
    row.innerHTML = `
      <td>${r.name} ${status}</td>
      <td>${start || `<button onclick="startRoutine(${index})">Start</button>`}</td>
      <td>${end || (start ? `<button onclick="endRoutine(${index})">End</button>` : "")}</td>
      <td>${duration}</td>
      <td>${noteInput}</td>
      <td>
        <button onclick="resetEntry(${index})" ${(!start && !end) ? "disabled" : ""}>Reset</button>
      </td>
    `;
